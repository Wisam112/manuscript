...
export class ManuscriptService {
  private manuscriptsUrl = "http://localhost:8080/api/manuscripts";
  ...
  createManuscript(newManuscript: Manuscript): Promise<void | Manuscript> {
    ...
    return this.http
      .post(this.manuscriptsUrl, newManuscript)
      .toPromise()
      .then(response => {
        console.log("(1.9) after service", response);
        return response as Manuscript;
      })
      .catch(this.handleError);
  }
  ...
  deleteManuscript(delManuscriptId: String): Promise<void | String> {
    return this.http
      .delete(this.manuscriptsUrl + "/" + delManuscriptId)
      .toPromise()
  ...
  uploadFiles(filesToUpload: Array<File>, manuscript: Manuscript = testDB[0]) {
    ...
    var uploadURL =
      ...
      ":8080" +
      `/upload/${manuscript.ownerId}/${manuscript.versionId}/${version}`;
