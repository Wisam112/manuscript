...
export class ManuscriptDetailsComponent {
  ...
  get fileUploadEnabled() {
    return (
      this.pendingFiles && this.pendingFiles.length != 0 && !this.inProgress
    );
  }
  get isOriginalVersion() {
    return this.manuscript._id === this.manuscript.versionId;
  }
  ...
  setVersion(versionId: string, updateFirst: boolean = this.autoSave) {
    if (!updateFirst) this.setVersionPostUpdate(versionId);
    else
      this.updateManuscript(this.manuscript).then(updatedManuscript => {
  ...
  updateManuscript(
    ...
    isNewManuscript = false
  ): Promise<Manuscript> {
    this.lock();
    var updatePromise = this.uploadFiles
      .bind(this)(manuscript)
  ...
  addFilesToManuscript(files: ManuscriptFile[], manuscript: Manuscript) {
    if (!manuscript.files) manuscript.files = [];
  ...
  createVersion(updateFirst: boolean = this.autoSave) {
    var copy: Manuscript = <Manuscript>(
      JSON.parse(JSON.stringify(this.manuscript))
    ); //deep copy.
    ...
    if (!updateFirst) {
      ...
      that
        .updateManuscript(this.manuscript) //save current version TODO add prompt
  ...
  //Form Validation
  isValid(field) {
    return this.inProgress || field.valid || field.pristine;
  }
  ...
  //File handling
  filterFiles(files) {
    return this.filesToArray(files).filter(file =>
      this.legalFormats.includes(file.name.split(".").pop())
    );
  }
  ...
  displayFileNames(
    files: any[] = this.pendingFiles,
    max: number = this.maxFileDisplay
  ) {
    this.showFiles = true;
    return files.length > max
      ? files.slice(0, max).concat([{ name: `${files.length - max} more...` }])
      : files;
  }
  ...
  uploadFiles(manuscript): Promise<ManuscriptFile[]> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.manuscriptService
        .uploadFiles(this.pendingFiles, manuscript)
        .subscribe(event => {
  ...
  deleteFiles(manuscript: Manuscript) {
    return this.manuscriptService
      .deleteFiles(this.markedForDelete)
  ...
  confirmExecFunc(prompt, funcToExec: Function, ...args) {
    ...
    if (this.autoSave || !this.isChanged() || this.isOriginalVersion)
      ...
      this.modalService.open(prompt).result.then(doUpdateFirst => {
        funcToExecBound(...args, doUpdateFirst);
      });
    }
  }
